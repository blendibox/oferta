...
export async function lerProdutoPorSlug(slug, nomeArquivoJsonl, lote) {
	
	//let filePath = path.join(process.cwd(), 'public', 'data', `${nomeArquivoJsonl}.jsonl`); 
	let filePath = path.join(process.cwd(), 'data', 'awin', `${nomeArquivoJsonl}.jsonl`); 
	
	if(lote){
		 filePath = path.join(process.cwd(), 'data', 'awin', `${nomeArquivoJsonl}_${lote}.jsonl`);
	}
