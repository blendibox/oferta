...
export async function generateStaticParams() {
   ...
   if (process.env.BUILD_TARGET !== 'calvinklein') {
  ...
  const slugsProduto = Object.entries(slugIndex)
    .filter(([_, arquivo]) =>  /^calvinklein\.json$/i.test(arquivo))
    .map(([slug]) => slug);
...
export async function generateMetadata({ params }) {
  const produtos = await lerProdutosJSON('CALVINKLEIN');
